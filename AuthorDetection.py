# -*- coding: utf-8 -*-
"""
Created on Tue Nov  5 02:31:04 2019
...
def main():
    ...
    text2 = "HM.txt"
    text3 = "JA.txt"
    tableACD = scanBook(text1)
    tableHM = scanBook(text2)
    tableJM = scanBook(text3)
    
    # combining table A and table H
    tableCombine2 = tableACD.append(tableHM, ignore_index=True)
    
    # combining table A, H and J
    tableCombine3 = tableCombine2.append(tableJM, ignore_index=True)
    # Assigning the value of tables in list format
    twoAuthorList = tableCombine2.values[:, 0:7]
    threeAuthorList = tableCombine3.values[:, 0:7]
    
    Action = input('Enter 1,2,3: Accuracy to be predicted by 1. Decion tree, 2. logistic Classifier, 3.exit \n')
    
    while (Action != "3"):
        if Action == "1":
            numberOfAuthor = input("run with 2 or 3 authors \n")
            if numberOfAuthor == "2":
                train, testing_data, X_train, X_test, y_train, y_test = splitTrainTest(twoAuthorList)
            elif numberOfAuthor == "3":
                train, testing_data, X_train, X_test, y_train, y_test = splitTrainTest(threeAuthorList)
            ...
            numberOfAuthor = input("run with 2 or 3 authors \n")
            if numberOfAuthor == "3":
                logisticthree(threeAuthorList)
            else:
                logistic(twoAuthorList)
            Action = input('Enter 1,2,3: Accuracy to be predicted by 1. Decion tree, 2. logistic Classifier, 3.exit \n')
